// Deliberately separate from the live direct benchmark's evaluator.
const fs=require('node:fs'),vm=require('node:vm');
const [candidate,tests]=process.argv.slice(2);
const context=vm.createContext({}, {codeGeneration:{strings:false,wasm:false}});
const candidateContext=vm.createContext({}, {codeGeneration:{strings:false,wasm:false}});
vm.runInContext("const exports={};const module={exports};const console={log(){},warn(){},error(){}};",candidateContext);
vm.runInContext(
...
try{
 // Candidate globals/prototypes cannot replace the grader's assert/eq helpers.
 vm.runInContext(fs.readFileSync(candidate,'utf8'),candidateContext,{timeout:2000});
 context.exports=vm.runInContext('module.exports',candidateContext,{timeout:2000});
 const checks=JSON.parse(fs.readFileSync(tests,'utf8'));
 (async()=>{
  const results=[];
  for(const check of checks){
   try{await vm.runInContext('(async()=>{'+check+'})()',context,{timeout:2000});results.push({pass:true})}
